...
- A guard that cannot run does not break the workflow it guards: the managed
  git hooks distinguish a scan that reported a finding (the commit is blocked,
  as intended) from a scan that could not run at all (skipped, with the reason
  on stderr). A missing installation is `doctor`'s to report.
