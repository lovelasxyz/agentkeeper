...
| 5 | A broken install bricks `git commit` | **Open** | The managed hook chain invokes the global CLI unconditionally. When the package went missing from `node_modules` during a release cycle, every commit in the repository failed with `MODULE_NOT_FOUND` — agentkeeper taking a developer's git with it. The hook must skip its check and say so when the CLI is unavailable, rather than failing the commit. The same pattern as the rest of this document: the hook asserts the CLI exists instead of observing it. |
